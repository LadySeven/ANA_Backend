...
def create_callbacks(model_name = "audio_cnn"):
    ...
    return callbacks, model_path_keras, model_path_tflite


def train_model(data_dir):
    (x_train, x_test, y_train, y_test), label_encoder = load_dataset(data_dir)
    model = build_model(x_train[0].shape[1:], x_train[1].shape[1:], y_train.shape[1])

    callbacks, model_path_keras, model_path_tflite = create_callbacks()
    ...
    print(f"Model saved as: {model_path_keras} and {model_path_tflite}")
